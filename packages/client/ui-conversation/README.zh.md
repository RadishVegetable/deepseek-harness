...
其他 conversation 视图通过 `conversationChatView` client service 共享 Chat store、Host 操作和 ChatView 组件。该 service 将这些 value import 保留在本包内，因此 [`ui-tavern`](../ui-tavern/README.md) 可以组合标准 transcript 和 composer，而不产生第二个 client bundle 依赖或发送路径。
