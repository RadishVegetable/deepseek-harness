...
The root entry also declares the `tavern` surface slot. AppFrame renders that slot above the shared conversation when the document carries the explicit Tavern marker; the surface is absent from ordinary Web rendering.
