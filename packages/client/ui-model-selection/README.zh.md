...
模型选择插件（浏览器侧）：**三个入口共用一份会话级目录**，由 `ModelDirectoryResolver`（`ctx.modelDirectories`）持有。对于普通会话，`/model` popupSelect 贡献项（经 `ctx.commandUi` 注册）、composer 的具名 `conversation.input.model` slot，以及 Tavern 的 `tavern.settings.model` slot 都使用同一个 `ModelDirectory` 实例。Tavern 会把模型控件放在“旅程设置”抽屉中，并隐藏逐消息 composer 里的模型入口。紧凑型触发器会打开两级 Model/Effort 菜单：模型仍按提供方分组，所选具体模型则提供由其适配器持有的推理强度名称、说明和默认值。`/model` 应用所选模型的默认推理强度，其余入口随后可以选择任一已公布的推理强度。
...
间接影响。三个入口都通过仅供普通会话使用的 `session.selectModel` RPC 提交完整的 `ModelSelection`；Host 会在下一次提示词组装边界对其进行快照，因此后续请求采用所选提供方、模型与推理强度，而运行中的步骤保留已组装选择。只有当现有请求头记录一次实际采用该选择的请求后，选择才会持久化；菜单交互不会添加提示词内容。
