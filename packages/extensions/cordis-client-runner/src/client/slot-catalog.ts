...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  },
  {
    ...
    keyDomain: 'fixed by the owner\'s key table { [Kind in ChatNodeKind]: { node: ChatNode<Kind> } }, already taken: assistant-step, command, command-input, compaction, context, manual-compaction, model-retry, steering, tavern-greeting, tool-call, turn-error, turn-max-tokens, turn-tail, unknown, user, workflow-run',
    hookContext: 'string',
    ...
    occupants: [
      ...
      'client-ui-tavern TavernGreetingNodeView key \'tavern-greeting\'',
  ...
  },
  {
    ...
    kind: 'list',
    scope: 'session',
    summary: 'Action strip attached to one finalized direct user message.',
    doc: 'Action strip attached to one finalized direct user message.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    declaredBy: 'an entry in \'conversation.chat.node\' (client-ui-conversation), so it exists while that entry is mounted',
    occupants: [
      'client-ui-tavern (props: Omit<Parameters<typeof TavernEditAction>[0], \'tav… id \'tavern-edit-message\'',
    ],
    replaceRisk: 'none',
  ...
  },
  {
    ...
    occupants: [
      ...
      'client-ui-tavern TavernView id \'tavern\'',
  ...
  },
  {
    ...
    occupants: [
      ...
      'client-ui-tavern-app TavernApp',
    ],
    replaceRisk: 'shadows-shipped-ui',
  ...
  },
  {
    ...
    kind: 'single',
    scope: 'root',
    summary: 'Legacy Tavern contribution slot retained for the optional package.',
    doc: 'Legacy Tavern contribution slot retained for the optional package.',
    registerOptions: [],
    ownerProps: [
  ...
  },
  {
    ...
    kind: 'single',
    scope: 'session-maybe',
    summary: 'The current Tavern Journey body, owned by the Tavern root.',
    doc: 'The current Tavern Journey body, owned by the Tavern root.',
    registerOptions: [],
    ownerProps: [
    ...
    keyDomain: '',
    hookContext: '',
    slotInject: '{ tavernAssets: TavernAssetsRemote; sessions: ISessions }',
    declaredBy: 'an entry in \'root\' (client-ui-tavern-app), so it exists while that entry is mounted',
    occupants: [
      'client-ui-tavern-app TavernJourney',
    ],
    replaceRisk: 'shadows-shipped-ui',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'tavern.journey\', () => ctx.slots.register(\n      { name: \'tavern.journey\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-tavern-app/src/client/index.ts:16',
  },
  {
    key: 'tavern.settings.model',
    kind: 'single',
    scope: 'session',
    summary: 'Model control projected into Tavern\'s journey settings drawer.',
    doc: 'Model control projected into Tavern\'s journey settings drawer.',
    registerOptions: [],
    ownerProps: [
    ...
    hookContext: '',
    slotInject: '',
    declaredBy: 'an entry in \'conversation.view\' (client-ui-tavern), so it exists while that entry is mounted',
    occupants: [
      'client-ui-model-selection ModelSelect',
    ],
    replaceRisk: 'shadows-shipped-ui',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'tavern.settings.model\', () => ctx.slots.register(\n      { name: \'tavern.settings.model\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-model-selection/src/client/index.ts:44',
  },
  {
